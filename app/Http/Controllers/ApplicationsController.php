...
class ApplicationsController extends Controller
{
    ...
    public function applicationsprofile(Request $request) {
      ...
      $applicantData = User::where('email', 'mhd.naeem90@gmail.com')
    ->join('companies', 'users.id', '=', 'companies.user_id')
    ->leftJoin('lease_application_registrations', 'lease_application_registrations.company_id', '=', 'companies.id')
    ...
    ->select(
        ...
        'lease_application_registrations.id as application_id'
    )
        ->where('users.email', '=', $user_email)
        ->paginate(10);
        ...
        ->join('lease_application_registrations', 'users.id', '=', 'lease_application_registrations.user_id')->where('users.email', '=', $user_email)->count();
        $new_count=DB::table('users')
        ->join('lease_application_registrations', 'users.id', '=', 'lease_application_registrations.user_id')->where('users.email', '=', $user_email)->where('lease_application_registrations.challan_uploaded', '=', 0)->count();
          ...
          // =dd($applicantData);
                   return view('admin.applications.lease_applications',['applicantdata'=>$applicantData, 'coordinatedata'=>$coordinateData,'applicantcompleteData'=>$applicantCompleteData,'total_count'=>$total_count,'new_count'=>$new_count]);
...
//`fee_title`, `fee_account`, `bank_name`, `fee_amount`, `fee_amount_in_words`,
    ...
    ->get();
         ...
         ->where('challan_fees.id',7)->get();
                ...
                return view('admin.applications.generatechallans',[ 'appid'=>$app_id, 'barcode'=>$barcode1D,'ranAl'=>$randomAlphabets
                ,'user_email'=>$user_email,'challanData'=>$applicantData,'coordinates'=>$coordinates]);
    }
 ...
 //// End Generate Challans ////////////////////////////////////////////
    ...
    public function getchallanfee(Request $request){
        ...
        $challanfeeId = $request->input('challanfeeId'); // Get the challan fee ID from the request
        $applicationId = $request->input('applicationId'); // Get the application
        //$search = $request->query('gem');
       // dd('this is '.$applicationId);
 ...
 //// save challan fee information ////////////
    ...
    public function saveChallan(Request $request){
       ...
       // dd($validatedData['application_id']);
